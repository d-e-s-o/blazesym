...
impl<'mmap> SymbolTableCache<'mmap> {
    fn new(syms: ElfN_Syms<'mmap>, strs: Cow<'mmap, [u8]>) -> Self {
        Self {
            syms,
    ...
    B: BackendImpl<'bcknd>,
{
    ...
    fn parse_syms(&self, section: &str) -> Result<ElfN_Syms<'bcknd>> {
        let ehdr = self.ensure_ehdr()?;
...
trait BackendImpl<'bcknd> {
    fn read_pod_obj<T>(&self, offset: u64) -> Result<Cow<'bcknd, T>, Error>
    where
        T: Pod + 'bcknd;

    fn read_pod_slice<T>(&self, offset: u64, count: usize) -> Result<Cow<'bcknd, [T]>, Error>
    where
        T: Pod + 'bcknd;
}

impl<'bcknd> BackendImpl<'bcknd> for &'bcknd [u8] {
    fn read_pod_obj<T>(&self, offset: u64) -> Result<Cow<'bcknd, T>, Error>
    where
        T: Pod + 'bcknd,
    {
...
impl<'bcknd> BackendImpl<'bcknd> for &File {
    fn read_pod_obj<T>(&self, offset: u64) -> Result<Cow<'bcknd, T>, Error>
    where
        T: Pod + 'bcknd,
    {

...
impl<'r> BackendRead<'r> for &'r Mmap {
    fn read_pod<T>(&mut self, offset: u64) -> Result<Cow<'r, T>, Error>
    where
        T: Clone + Pod,
    {
...
impl<'r> BackendRead<'r> for &'r File {
    fn read_pod<T>(&mut self, offset: u64) -> Result<Cow<'r, T>, Error>
    where
        T: Clone + Pod,
    {
...
impl<'r> BackendRead<'r> for &'r Backend {
    fn read_pod<T>(&mut self, offset: u64) -> Result<Cow<'r, T>, Error>
    where
        T: Clone + Pod,
    {
        match self {
            Backend::File(ref file) => {
                let mut file = file;
                file.read_pod::<T>(offset)
            }
            Backend::Mmap(ref mmap) => {
                let mut mmap = mmap;
                mmap.read_pod::<T>(offset)
            }
        }
    }
        ...
        T: Clone + Pod,
    {
        match self {
            Backend::File(ref file) => {
                let mut file = file;
                file.read_pod_slice::<T>(offset, count)
            }
            Backend::Mmap(ref mmap) => {
                let mut mmap = mmap;
                mmap.read_pod_slice::<T>(offset, count)
            }
        }
    }
}

    ...
    T::Ty32Bit: Clone,
{
    ...
    pub fn iter(&self) -> impl ExactSizeIterator<Item = ElfN<'_, T>> {
        match self {
            Self::B32(slice) => Either::A(slice.iter().map(|x| ElfN::B32(Cow::Borrowed(x)))),
            Self::B64(slice) => Either::B(slice.iter().map(|x| ElfN::B64(Cow::Borrowed(x)))),
        }
    }
}

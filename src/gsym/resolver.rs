...
use crate::symbolize::FrameSrcInfo;
use crate::Addr;
use crate::IntSym;
...
impl<'dat> GsymResolver<'dat> {
    ...
    fn query_frame_src_info(&self, file_idx: u32, line: Option<u32>) -> Result<FrameSrcInfo<'_>> {
        let finfo = self
            .ctx
    ...
    fn parse_line_tab_info(
        ...
        let frame = self.query_frame_src_info(lntab_row.file_idx, Some(lntab_row.file_line))?;
        Ok(Some(frame))
    }
}

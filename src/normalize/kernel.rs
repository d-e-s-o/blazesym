...
fn find_kcore_kernel_offset_note(parser: &ElfParser) -> Result<Option<&[u8]>> {
    ...
    for (idx, shdr) in shdrs.iter().enumerate() {
        ...
        let header = bytes.read_pod_ref::<Elf64_Nhdr>().ok_or_else(|| {
            Error::new(
                ErrorKind::InvalidData,
                "failed to read notes section header",
            )
        })?;
        let name = bytes
            ...
            .ok_or_else(|| {
                Error::new(
                    ErrorKind::InvalidData,
                    "failed to read build ID section name",
                )
            })?;
        // No point in checking `header.n_type`, as we have seen it be
        // simply 0 on valid kcore instances.
        if name.to_bytes() == b"VMCOREINFO\0" {
            let info = bytes.read_slice(header.n_descsz as _).ok_or_else(|| {
                Error::new(
                    ErrorKind::InvalidData,
                    "failed to read VMCOREINFO note section contents",
                )
            })?;
            return Ok(Some(info))
        }

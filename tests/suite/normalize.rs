...
fn normalize_custom_so_in_zip() {
    #[track_caller]
    fn test(so_name: &str, apk_to_elf: bool, build_ids: bool) {
    ...
    for (apk_to_elf, build_ids) in [(false, false), (true, false), (true, true)] {
        test("libtest-so.so", apk_to_elf, build_ids);
        test("libtest-so-no-separate-code.so", apk_to_elf, build_ids);
    }
}

...
/// A vector of field name-value pairs.
type FieldValues = Vec<(&'static str, Value)>;
...
impl Visitor {
    fn record_value(&mut self, field: &Field, value: Value) {
        let () = self.values.push((field.name(), value));
    }
...
#[derive(Clone, Debug)]
pub(crate) struct Builder<T = SystemTime, W = Stderr> {
...
impl<T, W> Builder<T, W> {
    ...
    pub fn with_time<U>(self, time: Option<U>) -> Builder<U, W> {
        let Self {
            verbosity,
            time: _,
            writer,
    ...
    pub fn build(self) -> Hierarchical<T, W> {
        let Self {
            verbosity,
            time,
            writer,
...
impl<T> Hierarchical<T, Stderr>
where
    T: FormatTime,
{
    fn write_names<W, S>(writer: &mut W, span: &SpanRef<'_, S>) -> fmt::Result
    where
        W: io::Write,
        ...
        F: FnOnce(&mut dyn io::Write) -> io::Result<()>,
    {
        ...
        if let Ok(()) = Self::write_impl(&mut writer, &self.time, level, span.as_ref(), f) {
            let _result = self.writer.lock().write_all(writer.written());
        }
    }
}

impl<S, T> Layer<S> for Hierarchical<T, Stderr>
where
    S: Subscriber + for<'lookup> LookupSpan<'lookup>,
    T: FormatTime + 'static,
{
    ...
    fn on_new_span(&self, attrs: &span::Attributes<'_>, id: &span::Id, ctx: Context<'_, S>) {
        if let Some(span) = ctx.span(id) {
            // Capture a snapshot of the span's values so that they can be
            // referenced later on.
            let mut visitor = Visitor::default();
            let () = attrs.values().record(&mut visitor);
            let values = visitor.into_values();
            let mut ext = span.extensions_mut();
            let _prev = ext.replace(values);
        }
    }
    ...
    fn on_event(&self, event: &Event<'_>, ctx: Context<'_, S>) {
        ...
        let write_fn = |w: &mut dyn io::Write| {
            ...
            for (name, value) in visitor.into_values() {
                // Special case the field "message", only printing the value and
                // not the name.
                if name == "message" {
                    let () = write!(w, " {value}")?;
                } else {
                    let () = write!(w, " {name}={value}")?;
                }
            }
            Ok(())
        };
    ...
    fn on_enter(&self, id: &span::Id, ctx: Context<'_, S>) {
        if let Some(span) = ctx.span(id) {
            ...
            self.write_args(ctx.span(id), None, write_fn);
        } else {
